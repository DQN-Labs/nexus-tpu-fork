...
| `Qwen4ExpDecoderLayer` (delayed combine) | `layers.py::Qwen4ExpDecoderLayer::forward` | Same PLE-materialize → `combine_and_mix`/`mix` → attn → `mlp_hyper_connection.combine_and_mix` (delayed) → MLP ordering; same `is_moe_layer` rule |
